...
const OUTPUT_DIR = resolve(ROOT, '.local/launch-film/story-audio');
const MANIFEST_PATH = resolve(import.meta.dirname, 'dialogue.json');
const MODEL_ID = 'eleven_multilingual_v2';
const OUTPUT_FORMAT = 'mp3_44100_128';

const LINES = [
  { id: 0, speaker: 'Lulu', text: 'Hello! I haven’t seen you here before. What’s your name?', file: '00-lulu-introduction.mp3' },
  { id: 1, speaker: 'Brian', text: 'My name’s Brian!', file: '01-brian-name.mp3' },
  { id: 2, speaker: 'Lulu', text: 'Hi Brian! You look drenched. You can get an umbrella at 7-Eleven. Have a discount on me!', file: '02-lulu-umbrella.mp3' },
  { id: 3, speaker: 'Brian', text: 'Thanks! I was out for a run and got caught in the rain.', file: '03-brian-rain.mp3' },
  { id: 4, speaker: 'Lulu', text: 'Well, let’s get you dry first!', file: '04-lulu-dry-first.mp3' },
  { id: 5, speaker: 'Lulu', text: 'Hey Brian! You don’t look as wet today.', file: '05-lulu-second-visit.mp3' },
  { id: 6, speaker: 'Brian', text: 'Your umbrella was so helpful. Thanks so much!', file: '06-brian-umbrella-thanks.mp3' },
  { id: 7, speaker: 'Lulu', text: 'You’re welcome! Been out running again?', file: '07-lulu-running-again.mp3' },
  { id: 8, speaker: 'Brian', text: 'Yes! Much better weather this time.', file: '08-brian-better-weather.mp3' },
  { id: 9, speaker: 'Lulu', text: 'Then you might like this. Lululemon’s having an event today. Come check it out!', file: '09-lulu-event.mp3' },
  { id: 10, speaker: 'Brian', text: 'Good looking out, Lulu!', file: '10-brian-good-looking-out.mp3' },
  { id: 11, speaker: 'Lulu', text: 'That’s what I’m here for!', file: '11-lulu-here-for.mp3' },
  { id: 12, speaker: 'Lulu', text: 'Hello! I haven’t seen you here before.', file: '12-lulu-closing.mp3' },
];
...
const VOICES = {
  Lulu: {
    ...
    settings: { stability: 0.5, similarity_boost: 0.75, style: 0.15, use_speaker_boost: true, speed: 0.95 },
  },
  Brian: {
    id: 'nPczCjzI2devNBz1zQrb',
    name: 'Brian - Deep, Resonant and Comforting',
    settings: { stability: 0.45, similarity_boost: 0.78, style: 0.18, use_speaker_boost: true, speed: 1.0 },
  },
};
...
async function main() {
  ...
  VOICES.Lulu.id = process.env.ELEVENLABS_VOICE_ID;
  if (!VOICES.Lulu.id) throw new Error('ELEVENLABS_VOICE_ID is missing from .env.local.');
  if (VOICES.Lulu.id !== 'cgSgspJ2msm6clMCkdW9') throw new Error('Configured Lulu voice is not the documented Jessica voice; stopping.');
  ...
  const manifest = results.map(({ line, duration: audioDuration }) => ({
    ...
    speaker: line.speaker,
    text: line.text,
    file: `.local/launch-film/story-audio/${line.file}`,
    duration: audioDuration,
  }));
